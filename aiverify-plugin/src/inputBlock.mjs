...
export async function validateInputBlock(
  argv,
  meta,
  skipMDXValidation = false
) {
  const pluginDir = argv._pluginDir;
  ...
  if (!skipMDXValidation) {
    // validate input block mdx
    const { getMdxWidgetBundle } = await import("./bundler.mjs");
    try {
      const result1 = getMdxWidgetBundle(mdxFile)
      if (!result1) {
    ...
    // validate widget mdx
    try {
      const result2 = getMdxWidgetBundle(summaryFile)
      if (!result2) {
        console.log("MDX error");
        return false;
      }
...
export async function validateAllInputBlocks(argv) {
  ...
  for (let f of myfiles) {
    ...
    if (!(await validateInputBlock(argv, meta))) return false;
  }
