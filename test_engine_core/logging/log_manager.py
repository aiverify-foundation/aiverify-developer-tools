...
class LogManager:
    """
    The LogManager class comprises methods to create new log managers and store observations and outputs while in operation
    """
    ...
    def _create_logger_helper(
        ...
        try:
            ...
            # Add the file and stream handler to the instance, so it will write to file and output to stdout
            self._handlers.append(
                self._get_static_file_handler(self._filepath, "w", self._log_level)
            )
            ...
            print(f"Failed while creating logging path: {str(error)}")
            return False, None
    ...
    def _get_logging_level(self, level: str) -> int:
        ...
        if level == "info":
            ...
            logging_level = self._level

        return logging_level

    def _get_static_file_handler(
        self, log_filepath: str, file_mode: str, level: int
    ) -> logging.FileHandler:
        """
        A helper method to interact with the python static file handler module
        ...
        Returns:
            logging.FileHandler: log filehandler instance
        """
        file_handler = logging.FileHandler(
            log_filepath, mode=file_mode, encoding=None, delay=False
        )
