...
export function generateWidget(argv) {
  ...
  for (let key of ["name","description","tag","minW","minH","maxW","maxH"]) {
    if (key in argv) {
      switch (key) {
        case 'minW':
        case 'minH':
  ...
  copyAndRenameTemplates(path.join(srcDir,"widgets"), "sample-widget", widgetDir, argv.cid)
}
...
export async function validateWidget(argv, meta, skipMDXValidation=false) {
  ...
  try {
    ...
    if (!res.valid) {
      console.error(`Widget meta for ${meta.cid} has validation errors:`, res.errors);
      return false;
    }
  } catch (err) {
...
export async function validateAllWidgets(argv) {
  ...
  for (let f of myfiles) {
    ...
    if (!(await validateWidget(argv, meta)))
      return false;
  }
